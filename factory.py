...
class ApplicationFactory:
    ...
    def create_question_answer_mapping(mapping):
        ...
        if ans_value == const.DEFAULT_QUESTION_TAG:
            ...
            answer = ApplicationFactory.create_answer(ans_value, tag, True, str(q_id) + ans_value)
            question.all_answer_options.append(answer)

    @staticmethod
    def create_answer(answer_value, answer_tag, use_id=False, a_id=None):
        answer = Answer.get_answer_by_id(a_id) if use_id else Answer.get_answer_by_value(answer_value)
        if answer is None:
            # create new answer
            answer = Answer(answer_value, answer_tag, use_id)
        return answer
